...
class VendorAuthSession extends Model
{
    use Cacheable;

    protected $table = 'vendor_auth_sessions';
    ...
    /**
     * Get the vendor that owns the session
     */
    ...
    public static function createForPhone(string $phone, ?array $registrationData = null): self
    {
        ...
        // Check if we can find a vendor id via vendor_employees
        $employee = VendorEmployee::findByPhone($phone);
        $vendorId = $employee ? $employee->vendor_id : null;

        // If not found via employee, check vendor table as fallback
        if (! $vendorId) {
            $vendor = Vendor::query()
                ->whereRaw(
