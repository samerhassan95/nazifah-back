        ...
        $orders = $query->with([
            ...
            ->map(function ($order) {
                ...
                $result = array_merge([
                    ...
                    'order_id' => $order->id,
                    'status' => $order->status,
                    'status_label' => OrderStatus::fromString($order->status)?->localizedLabel($order->payment_method, false, (bool) $order->delivery_at_vendor, true) ?? $order->status,
        ...
        if (! $assignmentType || ! $driverId) {
            ...
            return errorResponse(
                __('driver.no_driver_assignment_needed', [
                    'status' => $currentStatus?->localizedLabel($order->payment_method, false, (bool) $order->delivery_at_vendor, true) ?? $order->status,
                ]),
                [
                    'order_status' => $order->status,
                    'order_status_label' => $currentStatus?->localizedLabel($order->payment_method, false, (bool) $order->delivery_at_vendor, true) ?? $order->status,
                    'pickup_at_vendor' => (bool) $order->pickup_at_vendor,
                    'delivery_at_vendor' => (bool) $order->delivery_at_vendor,
                ],
                400
